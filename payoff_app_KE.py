...
st.caption("Multiplier e specifiche contrattuali sono caricati automaticamente dal catalogo e non appaiono nel grafico.")
st.subheader("Gambe")
legs = st.data_editor(
    st.session_state.legs_source, num_rows="dynamic", use_container_width=True, key="editor",
    column_config={
        ...
        "Premio/Ingresso":st.column_config.NumberColumn(step=.0001,format="%.4f",required=True),
        "IV %":st.column_config.NumberColumn(step=.1),
    })
...
if price_max<=price_min or legs.empty or legs[required].isnull().any().any() or analysis_date<valuation_date or not(show_analysis or show_now):
    st.error("Controlla range, date e dati obbligatori."); st.stop()
active=legs[~legs["Escludi"]].copy()
if active.empty: st.warning("Tutte le gambe sono escluse."); st.stop()

def expiry(row): return pd.to_datetime(row["Scadenza opzione"]).date()
...
def black76(f,k,t,s,r,typ):
    ...
    f=np.maximum(f,1e-12); vs=s*np.sqrt(t); d1=(np.log(f/k)+.5*s*s*t)/vs; d2=d1-vs; d=np.exp(-r*t)
...
def leg_pnl(p,row,target):
    ...
    return q*(black76(p,float(row["Strike"]),t,atm_iv/100,risk_free/100,row["Tipo"])-entry)*multiplier
def total(p,target): return sum((leg_pnl(p,row,target) for _,row in active.iterrows()),np.zeros_like(p,dtype=float))-commissions
def be(x,y):
...
def pop():
    ...
    s=atm_iv/100; z=max(12*s*np.sqrt(t),.25); low=max(future_price*np.exp(-z),1e-8); high=future_price*np.exp(z)
    edges=np.concatenate(([0],np.geomspace(low,high,12000),[np.inf])); q=(np.log(edges[1:-1]/future_price)+.5*s*s*t)/(s*np.sqrt(t)); mass=np.diff(np.concatenate(([0],cdf(q),[1])))
...
prices=np.linspace(price_min,price_max,2000); now=total(prices,valuation_date); ana=total(prices,analysis_date); nowbe,anabe=be(prices,now),be(prices,ana)
left,right=st.columns([3,1])
with left:
    fig=go.Figure()
    if show_components:
        for _,r in active.iterrows():fig.add_trace(go.Scatter(x=prices,y=leg_pnl(prices,r,analysis_date),mode="lines",opacity=.3,line={"dash":"dot"},name=f"{r['Lato']} {r['Tipo']} {r['Strike']}"))
...
with right:
    st.subheader("Metriche");st.metric("Sottostante",ticker);st.metric("Gambe incluse",f"{len(active)} / {len(legs)}");st.metric("PoP teorica data analisi",f"{pop():.1f}%")
...
levels=sorted(set([price_min,future_price,price_max]+active.loc[active["Strumento"]=="Opzione","Strike"].astype(float).tolist()+nowbe+anabe));sc=pd.DataFrame({"Prezzo":levels})
if show_analysis:sc["P/L data analisi"]=total(np.array(levels),analysis_date)
if show_now:sc["P/L At Now"]=total(np.array(levels),valuation_date)
st.subheader("Scenari");st.dataframe(sc.style.format({c:"{:.2f}" for c in sc.columns}),use_container_width=True)
