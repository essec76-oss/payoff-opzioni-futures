...
base=st.session_state.legs.copy().rename(columns={'Acquisto/Vendita':'Buy/Sell','Numero opzioni':'N° opz'});base['Oscura']=base.get('Oscura',False);base['Oscura']=base['Oscura'].fillna(False).astype(bool);base['Del']=base.get('Del',False);base['Del']=base['Del'].fillna(False).astype(bool);base=base[COLS];st.session_state.legs=base;active=base[(~base['Oscura']) & base.apply(valid,axis=1)].drop(columns=['Oscura','Del'])
