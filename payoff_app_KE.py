...
for k,v in {'project_name':'Strategia_opzioni','asset_class':'Futures Commodity','product':default,'spot':721.75,'atmiv':30.,'start':date(2026,8,7),'analysis':EXP,'rf':0.,'cash':0.,'pmin':450.,'pmax':1350.,'showa':True,'shows':True}.items(): st.session_state.setdefault(k,v)
...
with st.expander('Salva / Carica progetto'):
    ...
    if load and up:
        ...
        for k,v in z.get('parametri',{}).items():st.session_state[k]=pd.to_datetime(v).date() if k in ['start','analysis'] else v
        st.rerun()
...
r2=st.columns(6);start=r2[0].date_input('Data partenza',key='start');analysis=r2[1].date_input('Data analisi',key='analysis');atmiv=r2[2].number_input('ATM IV (%)',min_value=.01,step=.1,key='atmiv');rf=r2[3].number_input('Tasso risk-free (%)',min_value=0.,step=.1,key='rf');pmin=r2[4].number_input('Range minimo',step=1.,key='pmin');pmax=r2[5].number_input('Range massimo',step=1.,key='pmax');rate=rf/100;st.divider()
base=st.session_state.legs.copy().rename(columns={'Acquisto/Vendita':'Buy/Sell','Numero opzioni':'N° opz'});base['Del']=base.get('Del',False);base['Del']=base.Del.fillna(False).astype(bool);base=base[COLS];st.session_state.legs=base;active=base[base.apply(valid,axis=1)].drop(columns='Del')
t1,t2,t3=st.tabs(['Strategia','Comparazione 1','Comparazione 2'])
with t1:
    display=pd.concat([base,base.apply(lambda r:calc(r,start),axis=1)],axis=1);display['P&L AtNow']=display.apply(lambda r:pnl(np.array([spot]),r,start)[0] if valid(r) else np.nan,axis=1);display['P&L scad.']=display.apply(lambda r:pnl(np.array([spot]),r,analysis)[0] if valid(r) else np.nan,axis=1)
    with st.form('editor'):
        edited=st.data_editor(display,num_rows='dynamic',use_container_width=True,disabled=['DTE','xTM','Delta','Gamma','Vega','THETA','Pr. opz.','V. Temp.','V. Intr.','P&L AtNow','P&L scad.'],column_config={'Del':st.column_config.CheckboxColumn('Del'),'Buy/Sell':st.column_config.SelectboxColumn('Buy/Sell',options=['Buy','Sell']),'Call/Put':st.column_config.SelectboxColumn('Call/Put',options=['Call','Put']),'Scadenza':st.column_config.DateColumn(format='DD/MM/YYYY')});update=st.form_submit_button('Calcola payoff',type='primary')
    ...
    if not active.empty:
        g=active.apply(lambda r:calc(r,start),axis=1)[['Delta','Gamma','Vega','THETA']].sum()
        ...
        if st.session_state.showa:fig.add_trace(go.Scatter(x=x,y=ya,name='P/L data analisi',line={'width':4,'color':'#00a878'}))
        if st.session_state.shows:fig.add_trace(go.Scatter(x=x,y=ys,name='P/L data partenza',line={'dash':'dash','color':'#3b82f6'}))
        ...
        q=st.columns([1.3,1.3,5]);q[0].checkbox('P/L data analisi',key='showa');q[1].checkbox('P/L data partenza',key='shows')
with t2:st.info('Comparazione 1 predisposta.')
with t3:st.info('Comparazione 2 predisposta.')
out=st.session_state.legs.drop(columns='Del').copy();out.Scadenza=out.Scadenza.apply(lambda v:pd.to_datetime(v).date().isoformat() if pd.notna(v) else None);safe=re.sub(r'[^A-Za-z0-9_-]+','_',project_name).strip('_') or 'strategia_opzioni';data={'opzioni':out.to_dict('records'),'parametri':{'project_name':project_name,'asset_class':asset,'product':selected,'spot':spot,'atmiv':atmiv,'start':start.isoformat(),'analysis':analysis.isoformat(),'rf':rf,'cash':cash,'pmin':pmin,'pmax':pmax,'showa':st.session_state.showa,'shows':st.session_state.shows}};slot.download_button('Salva progetto',json.dumps(data,ensure_ascii=False,indent=2).encode(),f'{safe}_{date.today().isoformat()}.json','application/json')
